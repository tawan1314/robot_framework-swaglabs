...
Test Tags    website    swag_labs    sort_feature    sl004    positive    data_driven


*** Test Cases ***                                            USERNAME              PASSWORD              SORT_OPTION               NUM_OF_ITEM              ITEMS_INFO
...
Sort item and verify all of items
    [Arguments]    ${USERNAME}    ${PASSWORD}    ${SORT_OPTION}    ${NUM_OF_ITEM}    ${ITEMS_INFO}
    login_feature.Login successfully    ${USERNAME}    ${PASSWORD}
    products_feature.Verify and sort items    ${SORT_OPTION}
    products_feature.Verify sorted items      ${NUM_OF_ITEM}    ${ITEMS_INFO}
